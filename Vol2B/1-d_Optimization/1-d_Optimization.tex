...
\label{lab:1-dOpt}
\objective{There are several canonical techniques for optimizing scalar-valued functions.
In this lab, we implement the golden ratio method, the bisection algorithm, Newton's method, and the secant method.
We will generalize some of these approaches to optimization in $n$ dimensions in future labs.
}
...
At each step, the interval is reduced by a factor of $1-\rho$, which means that after $n$ steps, we have pinned down the minimizer to within an interval approximately $(0.61803)^n$ times the length of the original interval.
...
The SciPy module \li{scipy.optimize} contains implementations of various optimization algorithms, including several line search methods.
