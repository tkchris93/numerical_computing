...
To generate a JSON message, we use \li{dump}. This method accepts a Python object, generate the message, and writes it to a file.
The method \li{dumps} does the same, but returns the string instead of writing it to a file.
To perform the inverse operation, we use \li{load} or \li{loads} to read a file or string respectively.
