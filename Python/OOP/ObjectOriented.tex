...
You can see all of the available magic methods for a \li{Backpack} object with the following code:
...
In all of our preceding examples, the comments enclosed by sets of three double quotes are stored as the object's \emph{docstring}, stored as \li{__doc__}.
...
% Problem 3: __str__ magic method
\begin{problem}
The \li{__str__} magic method is used to give the string representation of an object.
When the object is used with the \li{print} command, the \li{__str__} method is called.
Add the \li{__str__} method to Backpack.py so that \li{print backpack} yields
\begin{lstlisting}
 Name:		<name>
 Color:		<color>
...
\begin{problem}
Create a \li{ComplexNumber} class that supports the basic operations of complex numbers. Implement a \li{conjugate} method that return a \li{ComplexNumber} object that is the complex conjugate, and a \li{norm} method that returns the magnitude of the complex number.
