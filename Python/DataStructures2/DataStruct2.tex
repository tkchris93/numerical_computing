...
%For example, the recursive method in the preceding problem is subject to failure if the \li{LinkedList} object holds more than 1000 items.
...
Use the \li{iterative_search} function from problem 1 to search the \li{LinkedList} and \li{find} so search the trees.
Calculate the average search time for each object.

In the first subplot, plot the number of words in each data set against the time it took to build each of the tree objects.
In the second subplot, plot the number of words in each data set against the average time it took to search the \li{BST} and the \li{AVL} objects.
