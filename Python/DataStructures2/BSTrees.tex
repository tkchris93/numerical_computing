...
\item The target is in the tree but is not the root:
	...
	\end{enumerate}
\end{enumerate}
(Hints: \textbf{Before coding anything} outline the entire function with comments and \li{if else} statements.
...
Choose 5 \textbf{random} words from the subset, and time how long it takes to find all 5 words in each object. To avoid recursive overflow, use the provided \li{iterative_search()} function from Problem \ref{prob:recursion} to search the linked list.

Create a single plot with two subplots.
In the first, plot the number of words in each subset against the time it took to build each object.
In the second, plot the number of words in each subset against the time it took to search each object.
