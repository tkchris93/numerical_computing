...
\foreach \s/\t in {2a/1a, 1b/2b, 2a/4a, 4b/2b, 5a/2a, 2b/5b, 5a/9a, 9b/5b} 
    {\draw[->, >=stealth', shorten <=.23cm, shorten >=.1cm](\s)--(\t);}
\end{tikzpicture}
\caption{To remove the node containing 2 from the top left BST, start `current' at the root and recurse down the tree until it points to the target. Then locate the in-order successor and delete it, recording its data. Finally, swap the data in the target with the data that was in the successor.}
