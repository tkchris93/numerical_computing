...
\node [draw=none, black!20!blue, node distance=1.5cm](root)[above right of=1a]{root};
\draw[black!20!blue, ->, >=stealth', shorten >= .1cm](root)--(1);
...
Time (separately) how long it takes to load a \li{LinkedList}, a \li{BST}, and an \li{AVL} with the $n$ items.
%Use \li{append()} to load the \li{LinkedList} and \li{insert()} to load the trees.
Then choose 5 \emph{random} words from the data set, and time how long it takes to find each word in each object.
Use the \li{iterative_search()} function from problem 1 to search the linked list and \li{find()} to search the trees.
...
%     \begin{subfigure}[t]
%         %\centering
%         \includegraphics[width=0.5\textwidth]{BuildTimes.pdf}
%         \caption{This is a caption.}
%     \end{subfigure}%
%     ~
%     \begin{subfigure}[t]
%         %\centering
%         \includegraphics[width=0.5\textwidth]{SearchTimes.pdf}
