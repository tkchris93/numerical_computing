...
% TODO Provide a quick example of addition and element-wise multiplication
...
\end{lstlisting} 
Some other commonly used functions are \li{np.random.normal}, which samples from the normal distribution, \li{np.random.randint}, which randomly selects integers from a range, and \li{np.random.random_integers} which returns an array of random integers in a given range.
