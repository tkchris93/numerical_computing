...
In addition to \li{np.array()}, NumPy provides efficient ways to create special kinds of arrays. The function \li{np.arange([start], stop, [step])} returns an array of numbers from \li{start} to, but not including, \li{stop}. Like other function that have similar parameters, \li{step} 
