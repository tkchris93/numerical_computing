...
Most data sets can be visualized in many forms and styles.
There is usually no ``best'' or inherently correct way to visualize data, but there are several good principles to follow (and several practices to avoid).

\begin{enumerate}
    \item \textbf{Clarity}.
    Visualizations should be self-explanatory.
    Always use specific titles and axis labels, and include units of measure.
    Use a legend for plots with more than one piece of information.

    % Where appropriate, use text annotations to carefully explain the major insights represented in the visualization.

    \item \textbf{Simplicity}.
    A good visualization communicates everything that it needs to, but nothing more.
    Unnecessary elements of a graph should be removed.
    % Don't add 3D effects in for no reason.

    \item \textbf{Integrity}.
    Tell the truth, the whole truth, and nothing but the truth.
    It is almost always very easy to manipulate a visualization so that it supports a desired interpretation.
    Resist the temptation to twist a picture into what you personally want it to be.

    Finally, every visualization should be presented alongside all relevant information including who created it, where the data was obtained, how it was collected, whether it was cleaned or transformed, and whether there are conflicts of interest or possible biases present.
    In short, cite your sources.
\end{enumerate}
...
Some of this function's keyword arguments are summarized below.

\begin{table}[H]
\begin{tabular}{r|c|l}
    \multicolumn{1}{c|}{Argument} & Options & \multicolumn{1}{|c}{Description}
    \\ \hline
