...
A heat map of $f$ assigns a color to each $(x,y)$ point in the domain based on the value of $f(x,y)$, while a contour plot is a drawing of the level curves of $f$. % TODO: recap level curves (there's space to do it now).
...
These functions receive the locations of each bar, then the height of each bar.
...
Data in a bar chart should also be sorted in a logical way, either by bar size or by alphabetizing the labels.
...
\begin{problem}
The file \texttt{countries.npy} contains information about 20 countries.
