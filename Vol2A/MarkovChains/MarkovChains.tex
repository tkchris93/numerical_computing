...
A single draw from a \emph{binomial distribution} with parameter $p$ indicates the outcome of a single experiment with probability $p$ of success.
...
# Make a single draw from the binomial distribution with p = .5 (flip a coin).
...
To determine whether tomorrow is hot or cold, draw from the binomial distribution with parameter $p = 0.3$.
...
A single draw from a multinomial distribution with $n$ parameters $(p_1,\ p_2,\ \ldots,\ p_n)$ indicates which of $n$ outcomes occurs.
In this case the classic example is a dice roll, with $6$ possible outcomes instead of the $2$ possible outcomes of a coin toss.

\begin{lstlisting}
>>> die_probabilities = np.array([1./6, 1./6, 1./6, 1./6, 1./6, 1./6])

# Make a single draw from the multinomial distribution (roll a die).
...
To accommodate larger data sets, consider use a sparse matrix for the transition matrix in instead of a regular NumPy array. %(use the \li{lil_matrix} from the \li{scipy.sparse} library). % TODO: Why lil_matrix?
