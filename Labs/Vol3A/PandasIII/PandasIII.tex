...
With a given pandas \li{DataFrame}, we can visualize data easily with the method \li{pivot_table()}. The Titanic dataset (\li{titanic.csv}) is especially apt for this.  It includes many columns, but we will use only \li{Survived}, \li{Pclass}, \li{Sex}, \li{Age}, and \li{Fare} here.  You will need to load in this dataset for upcoming problems, using principles taught in Lab \ref{lab:pandas1}. Note that many of the ages are missing values.  For our purposes, simply fill these missing values with the average age.  Then drop any rows that are missing data. Once we have a usable dataset, we can make Pivot Tables to view trends in the data.
...
Now with this other partition of the column age, we can add this dimension to our pivot table, passing it along with \li{Sex} in a list to the parameter index.
