...
This function should accept $A$, $\b$, and $\c$ as parameters, along with the keyword argument \li{niter} that gives the number of iterations to perform (default this value to $20$).
...
\begin{problem} % Second step: define DF(x,lamb,mu) ---------------------------
Within \li{interiorPoint()}, define a function for the matrix $DF$ described above.
This function should take $\x$ and $\Mu$ as parameters and return a $(2n+m) \times (2n+m)$ NumPy array.
\end{problem}
...
\begin{problem} % Third step: Search direction --------------------------------
...
\begin{problem} % Fourth Step: Step size --------------------------------------
Within \li{interiorPoint()}, write a subroutine to compute the step size after the search direction has been computed.
Avoid using loops when computing $\alpha_{\max}$ and $\beta_{\max}$ (use of masking and NumPy functions instead).
\end{problem}
...
Use the function provided above to select an initial point, then run the iteration \li{niter} times.
Return the optimal point $\x^*$ along with the value of the objective function at this point.
...
\section*{Least Absolute Deviations} % APPLICATION ============================
