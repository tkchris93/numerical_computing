...
% Bold mu, lambda, and beta symbols for this lab.
...
Next, define
\[
\mu_a := \frac{1}{n}(x+\alpha_a^p\triangle \x^a)^T(s+\alpha_a^d\triangle \Mu^a),
\]
...
% Include a call to this function in your algorithm to obtain the initial point.
...
Use the function provided above to select an initial point, then run the iteration \li{niter} times, or until the duality measure is less than \li{tol}.
...
def randomLP(m):
    ...
    Inputs:
        m -- positive integer, number of desired constraints and
             the dimension of space in which to optimize.
    Outputs:
...
% NOTE: The randomLP() function listed above is a bit wimpy, since it only does
% 'square' linear programs. When we give interiorPoint() LP's with m > n, like
% m=7 and n=5, it fails about 5% percent of the time. What is wrong with the
% algorithm or the implementation? Can we explain it to students?
% Another problem: the function often fails on 'square' linear programs when
% the dimension gets too high (really not that high, only like 10 or so).
% It is a problem with randomLP(), or with interiorPoint()?

\begin{info}
Our implementation of the interior point method is pretty good, but for linear programs where the number of constraints $m$ is strictly greater than the number of decision variables $n$, it fails to get quite the right answer about 11\% of the time.
If you can discover why this is, fix it, and write up your results in a clear and intelligle way (aka IPython Notebook), then you will earn massive extra credit on this lab.
-Shane
\end{info}

% TODO: In the future, time interiorPoint() against a Simplex method. Have a
% problem to demonstrate that each is advantageous in different settings.
