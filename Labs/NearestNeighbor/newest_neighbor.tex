...
To insert a new node, find the correct insertion point by recursively descending as in the \li{find} method (ee figure \ref{fig:k-insert} for an example).
...
To test your function, use Scipy's built-in \li{KDTree} object. This structure behaves like the \li{KDT} class, but its operations are heavily optimized. To solve the nearest neighbor problem, initialize the tree with data, then `query' the tree with the target point. The \li{query} method returns a tuple of the minimun distance and the index of the nearest neighbor in the data.
