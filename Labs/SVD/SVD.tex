...
% - add references to the textbook.
%
...
\begin{problem}
Write a function \li{truncated_svd} that accepts a matrix A and an integer \li{k = None}. If \li{k = None}, calculate the compact SVD. If $k$ is an integer, calculate the truncated SVD.
Since only difference between these two processes is the number of singular values we keep, we only need to write one function.
