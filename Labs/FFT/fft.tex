...
%   - Recommended spec format: iPython Notebook.
...
SciPy has built-in tools to read and create \li{wav} files.
...
\caption{The soundwave of \li{tada.wav}.}
\label{fig:tada_sig}
\end{figure}
...
\begin{problem}
Add a method to the \li{Signal} class called \li{export} that accepts a file name and generates a \li{.wav} file from the sample rate and the array of samples.
...
The SciPy/NumPy functions \li{hstack} and \li{vstack} may also be helpful.)
\end{problem}
