...
\objective{Implement a breadth-first search with adjacency dictionaries, learn to use NetworkX, and use the parlor game ``the Six Degrees of Kevin Bacon'' as an application to graph theory.}
...
% Problem 1: __str__ for Graph class (intro to dictionaries)
\begin{problem}
...
% Recall that a queue is a type of limited-access list. Remove from front, insert to back. Refer back to the end of Data Structures 1 for more info.

%\begin{comment} % TODO: put this in DataStructures1
...
The \li{collections} module in the Python standard library has a \li{deque} object.
The \li{popleft} method removes the first item in the deque, and the \li{append} method adds a new item to the end of the deque.
If we restrict ourselves to these two operations, we effectively have a queue.
\end{itemize}
%\end{comment}

\begin{comment} % waaaaay too long and unnecessary.
\subsection*{Queues and Deques}

A \emph{queue} is a kind of list that restricts data access and insertion.
New nodes are added to the end of the queue, but an existing node can only be removed or accessed if it is at the front of the queue.
Therefore, queues obey the rule of first-in, first-out (FIFO).
...
Since the ordering doesn't matter for which nodes are marked, we make marked a Python \li{set}.
