...
\begin{problem}
Implement the trust-region algorithm (see the specifications file). % using the following function signature.
% \begin{lstlisting}
% def trustRegion(f,grad,hess,subprob,x0,r0,rmax,eta=1./16,gtol=1e-5):
%     pass
% \end{lstlisting}
At this stage, do not solve Equation (\ref{eq:step}) for $p_k$.
Instead, assume the parameter \li{subprob} is a function that does that for you (see Problem \ref{prob:dogleg})
The function \li{subprob()} takes as parameters a gradient vector, hessian matrix, and radius.
\end{problem}
