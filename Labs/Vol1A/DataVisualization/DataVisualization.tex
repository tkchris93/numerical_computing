...
Different data sets are best visualized in different ways.
In this lab we discuss the principles of good data visualization, including the key characteristics of good graphs, common mistakes, and how to avoid unethical visualizations.}
...
Below we review three common ways to visualize one or two 1-dimensional arrays:
...
Displays for two corresponding arrays without a natural order, where the $(x,y)$ points are individual non-related instances.
...
\item \textbf{Histogram}: Groups $x$ values into a number of bins over an interval and displays the number of values in each bin.
...
\multicolumn{1}{|c|}{Bar Chart} & \multicolumn{1}{|c|}{Scatter Plot} \\
\cline{2-3}
\end{tabular}
\end{table}
...
% Get a marker and a line.
Line plots and scatter plots are sometimes used together.
A line connecting points implies a relationship between the points, while drawing point markers emphasizes that each point is a unique instance of data.

Line plots should \emph{not} be used for data without a natural progression.
For example, suppose we would like to make a large number of random draws from a distribution and visualize the results.
In this case, the line plot is completely incoherent, but the histogram provides an accurate picture of the statistical distribution.
...
>>> data = np.random.exponential(size=10000)\begin{lstlisting}
...
>>> plt.plot(bin_centers, counts, 'b.-', lw=2, ms=12)
...
\begin{subfigure}{.325\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/line_bad.pdf}
\end{subfigure}
%
\begin{subfigure}{.325\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/histogram_good.pdf}
\end{subfigure}
%
\begin{subfigure}{.325\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/histogram_line.pdf}
\end{subfigure}
\end{figure}

% Example of using np.histogram() for histogram with ordered data.

\begin{figure}[H] % line plot versus a histogram.
    \centering
    \includegraphics[width=.7\textwidth]{figures/histogram_line.pdf}
\end{figure}
...
\subsubsection*{Visualizing 2-D Surfaces} % -----------------------------------
