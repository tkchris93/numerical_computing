...
\begin{comment} % This just gives them a little too much example code . . .
\begin{info} % np.polyfit()
The fastest way to do least squares polynomial fitting with NumPy is with \li{np.polyfit()}.
This function receives an array of $x$ values, an array of $y$ values, and an integer for the polynomial degree; uses \li{np.vander()} to construct Equation \ref{eq:polynomial-least-squares}; and returns the coefficients of the best-fit polynomial, which can then be used with \li{np.poly1d()} or \li{np.polyval()}.%
\footnote{You may use \li{np.polyfit()} for Problem 3 if you're feeling lazy.}

\begin{lstlisting}
>>> x = np.linspace(-2, 4, 20)
>>> y = x**2 - 3*x + 1 + np.random.randn(20)

>>> coeffs = np.polyfit(x, y, deg=2)
>>> domain = np.linspace(-2, 4, 200)
>>> plt.plot(x, y, 'k*')
>>> plt.plot(domain, np.polyval(coeffs, domain), lw=2)
>>> plt.show()
\end{lstlisting}
\end{info}
\end{comment}
...
As a final example, consider a set of $m$ points $\{(x_k, y_k)\}_{k=1}^m$ that are arranged in a nearly circular pattern.
The equation of a circle with radius $r$ and center $(c_1,c_2)$ is as follows:
\begin{equation}
(x-c_1)^2 + (y-c_2)^2 = r^2.
\label{eq:standard-circle}
\end{equation}

The circle is completely determined by $r$, $c_1$, and $c_2$, so these are the parameters that should be the in $\x$ of the least squares formulation of the problem.
...
Expanding and rearranging Equation \ref{eq:standard-circle} gives the following:

\begin{align*}
(x-c_1)^2 + (y-c_2)^2 &= r^2 \\
x^2 - 2c_1x + c_1^2 + y^2 - 2c_2x + c_2^2 &= r^2 \\
x^2 + y^2 & = 2c_1x + 2c_2y + r^2 - c_1^2 - c_2^2 \\
x^2 + y^2 & = 2c_1x + 2c_2 y + c_3
\end{align*}
...
\begin{figure}[H]
    \includegraphics[width=.7\textwidth]{figures/circle.pdf}
...
Load the data in \texttt{circle.npy}.
