...
\footnote{See Abel's Impossibility Theorem.} % TODO: improve this footnote. Reference to proof in the book or the date when Abel first proved it?
...
If $A$ has a dominant eigenvalue $\lambda$, and if the projection of $\x_0$ onto the subspace spanned by the eigenvectors corresponding to $\lambda$ is nonzero, then the sequence of vectors $\x_0, \x_1, \x_2, \ldots$ converges
...
% If all entries of $A$ are positive, then $A$ will always have a dominant eigenvalue.%\footnote{Proof?}
...
% TODO: Algorithm box for the Power Method.

\begin{problem} % Implement the power method.
% Write a function that accepts an $n \times n$ matrix $A$, an error tolerance, and a number of maximum iterations.
% Use Algorithm \ref{} to compute the dominant eigenvalue and a corresponding eigenvector.
...
In mathematical notation, you are defining $x_0, x_1, \ldots x_k$, and your function should stop when $\|x_{k+1}-x_k\|$ is less than the tolerance.
\item Return the found eigenvector and the corresponding eigenvalue.
\end{enumerate}
