...
The QR decomposition of an $m\times n$ matrix $A$ is a factorization $A=QR$, where $Q$ is  has orthonormal columns and $R$ is upper triangular.
Every matrix $A \in M_{m\times n}$ of rank $n \le m$ has a QR factorization where $Q$ is $m\times m$ and $R$ is $m\times n$.
%
\footnote{In this lab we only discuss real matrices.
All of these results can be extended to the complex numbers by replacing ``transpose'' with ``hermitian conjugate'' and ``symmetric matrix'' with ``hermitian matrix.''}
...
The Gram-Schmidt algorithm computes an orthonormal basis $\{\q_i\}_{i=1}^n$ for the span of the $\x_i$.
...
\Procedure{Modified Gram-Schmidt}{$A$}
    ...
    \For{$i=0\ldots n-1$}
        ...
        \For{$j=i+1\ldots n-1$}
            \State $R_{i,j} \gets Q_{:,j}\trp  Q_{:,i}$
...
Note that \li{Q.dot(R)} does not equal \li{A} exactly because of small rounding errors.
However, \li{np.allclose()} reveals that the entries of \li{Q.dot(R)} are all approximately equal to the entries of \li{A}.
...
We can use the same method to check that \li{Q} is an orthogonal matrix.
...
\begin{problem} % Decomposition via Modified Gram Schmidt.
...
To test the algorithm, generate test cases with NumPy's \li{np.random} module.
...
Since $Q$ is orthonormal, $Q^{-1}= Q\trp$, so solving $A\x = \b$ is equivalent to solving the system $R\x = Q\trp\b$.
...
\begin{algorithm} % QR Decomposition via Householder Reflections.
\begin{algorithmic}[1]
...
However, \li{np.sign(0)} returns $0$, which will cause a problem in the rare case that $u_0 = 0$ (which is really only possible if the top left entry of $A$ is $0$ to begin with).
...
As with Problem \ref{prob:qr-via-mgs}, use \li{scipy.linalg.qr()} and \li{np.allclose()} to test your answers.
...
% A fast algorithm for computing the QR decomposition of a Hessenberg matrix will be taught in Lab \ref{lab:givens}. This algorithm in turn leads to a fast algorithm for finding eigenvalues of a matrix. % O RLY?
% % This is refering to the next lab, btw.
...
\Procedure{Hessenberg}{$A$}
    ...
    \For{$k=0 \ldots n-3$}
        ...
        \State $u_0 \gets u_0 + \sign(u_0)\|\u\|$
            \Comment{$u_0$ is the first entry of $\u$.}
        \State $\u \gets \u/\|\u\|$
