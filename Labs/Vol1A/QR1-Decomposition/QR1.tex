...
SciPy's \li{scipy.linalg.qr()} can compute the pivoted QR decomposition.
...
The QR factorization with pivoting is the basis for several rank-revealing algorithms and for handling rank-deficient least matrices problems (solving $Ax = b$ where $A$ is $m \times n$ with $m < n$).
...
This transformation is orthonormal (can you prove it?).
