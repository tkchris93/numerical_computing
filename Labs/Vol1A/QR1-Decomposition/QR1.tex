...
The best way to compute the full QR decomposition of a matrix is with a series of orthonormal transformations.
...
The \emph{orthogonal complement} of a nonzero vector $\v \in \mathbb{R}^n$, denoted $\v^\perp$, is the set of all vectors $\x \in \mathbb{R}^n$ that or orthogonal to $\v$.
In other words, $\v^\perp = \{ \x \in \mathbb{R}^n \mid \langle \x, \v \rangle = 0 \}$.
...
\end{tikzpicture}
\end{center}
\caption{The normal vector $\mathbf{v}$ defines the hyperplane $H$.
The Householder transformation $H_{\mathbf{v}}$ of $\mathbf{x}$ is the reflection of $\mathbf{x}$ across $H$.}
...
% % This is refering to the next lab, maybe to the QR Algorithm?
