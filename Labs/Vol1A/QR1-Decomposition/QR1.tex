...
Your function should use Algorithm \ref{Alg:modified-gram-schmidt}.
Generate test cases with \li{np.random.random()}, using \li{np.allclose()} to verify that $A = QR$.
\\
...
Use Algorithm \ref{Alg:QR-via-Householder} to compute the QR decomposition of $A$.
\\
(Hint: In steps \ref{step:HH-outer1} and \ref{step:HH-outer2} of Algorithm \ref{Alg:QR-via-Householder}, the multiplication of $\u$ and $(\u\trp X)$ is an \emph{outer product}---$\x\y\trp$ instead of the usual $\x\trp\y$.
