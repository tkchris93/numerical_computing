...
%
\begin{itemize}
    \item \textbf{Reduced QR}: $Q$ is $m \times n$, $R$ is $n \times n$, and the columns of $Q$ $\left\{\q_j\right\}_{j=1}^n$ span the column space of $A$.
    \item \textbf{Full QR}: $Q$ is $m \times m$ and $R$ is $m \times n$.
    In this case, the columns of $Q$ $\left\{\q_j\right\}_{j=1}^m$ form an orthonormal basis for $\mathbb{F}^n$, and the last $m - n$ rows of $R$ only contain zeros.
\end{itemize}
