...
The $s$ basis vectors of the new subspace, $\u_1, \ldots, \u_s$, are eigenvectors of $\bar{F}\bar{F}^T$.
...
The function should project the image or images into the appropriate $s$-dimensional subspace and return the result.
\item (Optional) Let \li{face} be the first mean-shifted face (the first column of \li{facialRec.Fbar}). Do the following:
\begin{enumerate}
