...
So if we start with a cake of size $W_0$ and have half the cake left at time $t$, then $V(t, T, \frac{W_0}{2})$ gives how much utility we will gain by proceeding optimally from $t$.
...
Write a method that accepts an integer $N$ representing the number of pieces we want in a discretized cake and a utility function.
...
\]
Return a matrix that whose $ij^{th}$ entry is the amount of utility gained by starting with $i$ cake pieces and ending with $j$ pieces.
...
We call this matrix the consumption matrix.
...
Thus, the amount of utility we gain from having $W_{T-1}$ cake is given by $u(W_{T-1})$, and so we know $V(T-1, T, W_{T-1})$ for any choice of $W_{T-1}$.
...
This will give an $N \times N$ matrix giving the value all possible values of starting with a cake of size $W_t$ and finishing with size $W_{t+1}$.
...
Thus, the maximal values of each row becomes the column of the value function matrix at time $t$ and if we know one column of the value matrix, we may iterate backwards to fill the rest of it in.
