...
\begin{problem} % Minimize a 'blackbox' function. TODO: rename blackbox().
...
The function \li{blackbox()} returns the length of a piecewise-linear curve between two fixed points: the origin, and the point \li{(40,30)}.
...
Though it may be difficult to see in the figure, the first six points skew the data, and do not help us determine the appropriate long-term values of $c$ and $\beta$. Thus, do not use the first six points when fitting a curve to the data.
