...
\begin{problem}
Write a function called \li{l1Min()} that takes a matrix $A$ and vector $b$ as inputs, and returns the solution to the optimization problem 
\begin{align*}
...
Seed NumPy's random number generator with \li{np.random.seed(1337)} before each measurement to obtain consistent results.

Resize and plot each reconstruction in a single figure with several subplots (use \li{plt.imshow()} instead of \li{plt.plot()}).
...
The single-pixel camera measurement process can be modeled as matrix multiplication by a measurement matrix $A$, which fortunately has the Restricted Isometry Property.
In the file \texttt{camera.py}, we provide you with code to take single pixel measurements.
\begin{lstlisting}
>>> from camera import Camera
>>> myCamera = Camera(faces, vertices, colors)
\end{lstlisting}
Where \li{faces}, \li{verticies} and \li{colors} are given by the tesselation.
...
% To take a picture, use the function
% \begin{lstlisting}
% myCamera.add_pic(theta, phi, r)
% \end{lstlisting}
% Where \li{theta}, \li{phi}, and \li{r} are the spherical coordinates of the camera.
...
% Calling \li{add_pic()} many times at different places on a sphere (constant $r$ value, varying $\theta$ and $\phi$) will build the measurement matrix as well as the measurements.
...
% In the the following problem $V$ will be given to you.
...
% Rubik's Cube problem (TODO: flesh this out and put it back in).
\begin{comment}
\begin{problem}
We will perform compressed sensing on a Rubik's cube.
The Rubik's cube has a natural tessellation, with each facet being a square having one color.
Each color can be represented as a vector of length 3, where we have split the color into three color channels (red, green, and blue).
The set of colors for the entire Rubik's cube is fairly sparse in each color channel, so we can reconstruct the color values in each color channel separately.

% TODO: Does StudentRubiksData.npz exist?
In the file \texttt{StudentRubiksData.npz}, we have given you the measurement matrices for each color channel (accessed by the keys \li{'A1', 'A2', 'A3'}) along with the respective measurements in each color channel (accessed by the keys \li{'b1', 'b2', 'b3'}).
Reconstruct the colors in each color channel, obtaining three arrays.
Then stack these arrays row-wise into an array with three rows. Report this final array.
...
Execute the following (assuming that the variable holding the array you obtained above containing the reconstructed colors is named \li{r}):
\begin{lstlisting}
...
% TODO: What is the matrix V for? It seems that you don't need it if you have StudentEarthData.npz.
\begin{problem} % Problem 3: Single-Pixel camera (Earth)
...
The file \texttt{StudentEarthData.npz} contains the faces, vertices, and colors, and the inverse fourier matrix, accessed by the keys \li{'faces'}, \li{'vertices'}, \li{'C'}, and \li{'V'}, respectively.
%Take pictures with the single pixel camera code (using $r=3$) and then
Use the faces, vertices, and colors to initialize a camera object as in the example code above.
Extract the data with the \li{returnData()} function.
