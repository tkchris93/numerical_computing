...
\objective{Python's NumPy and SciPy packages make Python an ideal language for scientific computing. NumPy is a package for manipulating data with multi-dimensional vectors, and SciPy is a higher-level library built on NumPy. Both packages will be used extensively in almost every lab hereafter in this curriculum. In this lab we introduce basic NumPy data structures and operations.}
...
%\footnote{SciPy is also the name of a Python coding environment that includes the NumPy and SciPy libraries, as well as IPython, matplotlib, and other tools.}
  ...
  %     Arrays
  %         What is an array?
  %         How to initialize a simple custom array
  %         * Problem 1: matrix multiplication
  %         SMALL Note about speed or Problem: Compare speed
  %         zeros(), ones(), full(), empty() (warning about empty()?)
  %         Basic array operations +, -, *, / component-wise
  %         Data Types
  %             How to initialize an array with a certain type
  %             Changing the type of an array's elements
  %             Compare to list comprehension to change types
  %         * Problem (?): Advantages of choosing the right type
  %     Array Manipulation: THIS SECTION NEED 10 MILLION PICTURES
  %         reshape(), flatten() / ravel()
  %             All 1D numpy arrays are flat!!!!!!
  %         hstack(), vstack(), row_stack(), column_stack()
  %         Slicing Revisited
  %         Fancy Indexing and Masks
  %             * Problem 2: Use fancy indexing to make the entries
  %                 of a matrix nonnegative (add more manipulations)
  %         Views vs Copies (Mutability) ???
  %             np.copy()
  %     Avoid Looping!!
  %         Again, NumPy uses C and Fortran to be SPEEDY!
  %         The 'axis' argument.
  %         all() and any()
...
We can specify the data type of an array as it is created with the \li{dtype} keyword argument, or change an array's type afterward with the array's \li{astype()} method.
...
In addition to casting other structures as arrays via \li{np.array()}, NumPy provides efficient ways to create ceratin specific arrays.
Each of these functions accepts the \li{dtype} keyword argument to specify the data type.

\begin{itemize}

\item \li{np.zeros()} and \li{np.ones()} create arrays of all zeros or all ones, respectively.
...
\item \li{np.arange()}, NumPy's version of the built-in \li{range()} function, creates evenly-spaced values within an interval specified with slicing syntax.
...
\item \li{np.linspace()} also create an array of evenly spaced values in a given interval.
...
\item The submodule \li{np.random} holds many functions for creating arrays of random values chosen from probability distributions such as the uniform, normal, and multinomial distributions.
...
% See \url{http://docs.scipy.org/doc/numpy/reference/routines.array-creation.html} for more details on array creation routines.
...
% THIS SHOULD BE MOVED TO THE COMPLEXITY LAB!!!
\begin{comment}
...
\begin{enumerate}
\item \li{np.empty()}: (from the docs:) Return a new array of given shape and type, without initializing entries. empty(), unlike zeros(), does not set the array values to zero, and may therefore be marginally faster. On the other hand, it requires the user to manually set all the values in the array, and should be used with caution.
\item \li{np.full()}: (from the docs:) Return a new array of given shape and type, filled with a given value. Equivalent (but faster than) \li{a*np.ones(shape)}.
\end{enumerate}

\subsection*{More on the Random Submodule}

Something about \li{np.random.choice()} and \li{np.random.randn()} ?
