...
\begin{lstlisting}
# The integers from 0 to 5.
>>> np.arange(5)
array([0, 1, 2, 3, 4])

# The integers from 3 to 8.
>>> np.arange(3, 8)
array([3, 4, 5, 6, 7])
...
\item \li{np.linspace(start, stop, num=50)} to create an array of \li{num} numbers evenly spaced in the interval from \li{start} to \li{stop}, \emph{inclusive}.
...
\item The submodule \li{np.random} holds many functions for creating arrays of random values chosen from probability distributions.
The most commonly used of these are:
\begin{enumerate}
\item np.random.random() or np.random.uniform()
\item np.random.randn()
\end{enumerate}

We can even create arrays of random values chosen from probability distributions.
These probability distributions are stored in the submodule \li{np.random}.
\begin{lstlisting}
...
Apart from being syntactically convenience, element-wise NumPy operations are significantly faster than element-wise list operations.
