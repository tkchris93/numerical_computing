...
Write a function that defines the matrices $A$ and $B$ given below as NumPy arrays and returns their matrix product.
%
\begin{align} \nonumber
...
For examples of array initialization and matrix multiplication, use IPython's object introspection feature to look up the documentation for \li{np.array()} and \li{np.dot()}.
\begin{lstlisting}
...
\begin{table}[H] % Important ndrray Attributes
\centering 
...
\item \li{np.arange([start], stop, [step])} returns an array of numbers from \li{start} up to, but not including, \li{stop}.
Like other functions with similar parameters, \li{start} defaults to $0$ and \li{step} defaults to $1$.
This is NumPy's version of the built-in \li{range()} function.
