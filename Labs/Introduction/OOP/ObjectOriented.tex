...
\begin{problem} % Problem 4: Add __hash__() to the Backpack class.
Two objects that compare as equal via the \li{__eq__()} magic method must have the same hash value, and objects that are not equal should have different hash values.

Write a simple \li{__hash__()} method for the \li{Backpack} class that conforms to this rule and returns an integer.
Consider using the \li{hash()} function on the object's attributes that were used in comparing the two backpacks.
\end{problem}

\begin{warn}
If a class does not have an \li{__eq__()} method it should not have a \li{__hash__()} operation either; if it defines \li{__eq__()} but not \li{__hash__()}, its instances will not be usable in hashed collections like sets and dictionaries.
\end{warn}

% From the docs:
% Custom classes have \li{__hash__()} methods by default; with them, all objects compare unequal (except with themselves) and \li{x.__hash__()} returns a result derived from \li{id(x)}.
...
\begin{tabular}{c|l}
Method & Corresponding Operation\\
% \hline \li{__repr__()} & Object representation (\li{repr()}).\\
