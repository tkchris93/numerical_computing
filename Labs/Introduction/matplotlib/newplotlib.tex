...
\begin{subfigure}{.5\textwidth}
    ...
    \label{fig:scatter}
\end{subfigure}
\caption{TODO: Replace this figure (or erase it).}
\end{figure}
\end{problem}
...
A \emph{heat map} visualizes such a function in only two dimensions by assigning the output of the function to a color (instead of a height).
For example, Figure \ref{fig:pcmexample} uses a heat map to graph $f(x,y) = \sin(x)\sin(y)$ on $[-6,6] \times [-6,6]$.
