...
The call \li{plt.show()} then displays the figure.

\begin{figure}[H] % TODO: add a subfigure on the right w/ correct x-axis.
\centering
\includegraphics[width=.5\textwidth]{basic.pdf}
\caption{A simple plot for $f(x) = x^2$, $x\in[-5,5]$.}
\label{fig:basic1}
\end{figure}
...
Now define a new function that creates an array of the results of the first function with $n = 100, 200, \ldots, 1000$.
\begin{lstlisting}
...
\subsection*{Specifying the Domain} % -----------------------------------------

An obvious problem with Figure \ref{fig:basic1} is that the $x$-axis does not reflect the actual values used to compute the array \li{y}.
To rectify this, we provide an additional array to serve as the domain.
Then \li{plt.plot(x,y)} plots \li{x} against \li{y}.
Hence both arrays must have the same shape.
...
\begin{figure}[H] % Basic line plots.
\centering
...
\begin{comment} % Replace this with Law of Large Numbers plot.
...
\begin{comment} % A little old, but might be useful to have a table like this.
...
\begin{comment} % Could still be a good problem if simplified.
