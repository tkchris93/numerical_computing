...
In this lab we will explore some built-in functions, become familiar with the standard library, and learn how to create, import, and use modules.}
...
%\hline \li{round()} & A float rounded number to a given precision in decimal digits. \\
%\hline \li{sorted()} & A list of an iterable's elements, sorted from least to greatest.
\end{tabular}
\caption{Some common built-in functions.}
\label{table:builtin}
\end{table}
% Other built-ins to include: any(), all(), map(), round(), type()
...
Documentation on all built-in functions can be found at \url{https://docs.python.org/2/library/functions.html}.
% Check out \li{map()}, \li{round()}, \li{pow()}, \li{reduce()}, \li{reversed()}, }
...
To see all of the names in the current namespace, use the built-in function \li{dir()}.% (in IPython, this list will be much longer).
To delete a variable from the namespace, use the \li{del} keyword.
...
Use \li{from} to specify the module where the object is located, then import it individually.
\begin{lstlisting}
...
% Problem 3: create your own module. % TODO: Something more creative!
\begin{problem}
...
% TODO: make a table listing some of the common modules in the standard library
%    (csv, math, random, string, sys, time, timeit)
