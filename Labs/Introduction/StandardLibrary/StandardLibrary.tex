...
The \li{all()} and \li{any()} functions are particularly useful for creating readable code when used in conjunction conditionals or list comprehensions.
...
Use the regular syntax to import a module that is in the current folder.
Use the syntax \li{from <package.module> import <names>} to load a module within a subpackage,
Once a name has been loaded into \texttt{ssb/\_\_init\_\_.py}, other files can load the same name with \li{from . import <name>}.
...
To access code in a file in the directory one level above the current directory, use the syntax \li{from .. imort <name>} or \li{from ..<module> import <name>} (no space after the two periods).
...
To run a package, run python from the shell with the flag \li{-m} (for ``module-name'') and exclude the extension \texttt{.py}.
