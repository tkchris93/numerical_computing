...
After reading a file once, either the file must be closed and reopened, or the cursor must be reset to the beginning of the file with \li{seek(0)} before it can be read again.

% Small files can usually be read once and stored entirely in memory.
...
Python's string type class, \li{str}, has several useful methods for parsing and formatting strings.
...
\begin{table}[H]
\begin{tabular}{r|l}
Method & \multicolumn{1}{c}{Returns} \\
\hline
...
This may seem like a trivial extension of the \li{Exception} class, but it is useful because the interpreter never automatically raises a \li{TooHardError}.
...
The \li{csv} module in the standard library has some methods that work with the file object to read a CSV file.
