...
Thus if a file cannot be opened for any reason, an \li{IOError} is raised and the file object is not initialized.
...
At the end of the block, the file is safely and automatically closed.
This is the preferred file-reading method when a file only needs to be accessed briefly, though it is not quite as flexible as the \li{try}-\li{finally} approach.
