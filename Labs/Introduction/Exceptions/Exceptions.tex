...
If an exception is raised, the interpreter exits the \li{try} block and enters the \li{except} block.
...
\begin{comment}
\begin{enumerate}
\item \li{try}: Put lines of code that might raise an exception here.
\item \li{except <Exception Class> as <e>}: Executed if an exception of the indicated class is raised anywhere in the \li{try} block.
\item \li{else}: Executed if an exception is not raised in the \li{try} block.
\item \li{finally}: Executed no matter what, even if there is a \li{return} statement or another exception is raised somewhere in the mix.
\end{enumerate}
\end{comment}

% TODO: Move this to "Additional Info" or delete.
\begin{lstlisting}
...
...         print strftime("%H:%M:%S")          # Print the current time.
...         return func(*args, **kwargs)        # Execute the inner function.
...     return wrapper
...
>>> @time_stamp
>>> def example():
