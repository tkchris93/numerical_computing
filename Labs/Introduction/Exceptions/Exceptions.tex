...
These arguments are stored as a an attribute called \li{args}, and also serve as the string representation of the object.
...
Sometimes, however, it is important to validate input types before they are utilized. TODO: Make \li{my_func()} more interesting.
\begin{lstlisting}
...
Other clauses can be attached to the \li{try}-\li{except} control flow.
% The interpreter's progression through the program is as follows.

\begin{description}
\item \li{try}: Executed until an exception is raised.
\item \li{except <Exception Class> as e}: Executed if an exception of the indicated class is raised anywhere in the \li{try} block. There may be multiple \li{except} statements that specify different kinds of exceptions to catch, and a single except statement may catch several kinds of exceptions.
\item \li{else}: Executed if an exception is \emph{not} raised in the \li{try} block.
\item \li{finally}: Always executed, regardless of any exceptions.
\end{description}
...
The code in the \li{finally} block is \emph{always} executed, even if a \li{return} statement or an uncaught exception occurs in any block following the \li{try} statement.
...
def random_walk(max_iters=1000000000):
    ...
    return walk
\end{lstlisting}
Modify \li{random_walk()} so that if the user raises a \li{KeyboardInterrupt} before completion, the function prints ``Process Interrupted at $x$\%'' (where $x$ is the percent completion through the loop), and returns the current value of \li{walk}.
If no \li{KeyboardInterrupt} is raised, the function should print ``Process Completed'' and return \li{walk} like usual.
\end{problem}
...
Files can be opened and closed safely with a careful \li{try}-\li{finally} control flow. % Make sure this is right!
...
This is the preferred method when a file only needs to be accessed briefly, but it is not quite as flexible as the \li{try}-\li{except}-\li{finally} approach.
