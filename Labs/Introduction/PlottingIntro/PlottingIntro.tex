...
The following code creates an array of outputs of the function $f(x) = x^2$, then visualizes the array using the \li{matplotlib} module.\footnote{Like NumPy, Matplotlib is \emph{not} part of the Python standard library, but is included in most Python distributions.}
...
Specify the number of bins with the argument \li{bins} and the range of values with the argument \li{<<range>>} to control the precision of the plot.

\item A \emph{scatter plot} plots two 1-dimensional arrays against each other without drawing lines between the points.

To create a scatter plot, use \li{plt.scatter()}, or use \li{plt.plot()} and specify a point marker (such as \li{'o'} or \li{'+'}) for the line style.
\end{itemize}
...
\begin{subfigure}{.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{scatterplot.pdf}
\end{subfigure}
\end{figure}
...
\begin{itemize}
\item A \emph{heat map} assigns a color to each entry in the matrix, producing a 2-D picture describing a 3-D shape.
...
\begin{subfigure}{.33\textwidth}
    \centering
    \includegraphics[width=\linewidth]{contourf.pdf}
\end{subfigure}
\end{figure}
...
Choose an appropriate number of level curves.
\item Set the limits of each subplot to $[-2\pi,2\pi] \times [-2\pi,2\pi]$.
\item Choose a non-default scheme (\li{"Spectral"} and \li{"viridis"} are both popular).
