...
\footnote{\li{plt.scatter()} can also be used to create scatter plots, but it accepts slightly different arguments than \li{plt.plot()} and is often less flexible.}
...
Each of the 148,206 rows in the array represents a different crash.
...
Label both axes.
\\
(Hint: the command \li{plt.gca().set_aspect("equal")} fixes the scaling of the axes so the picture is not skewed.)
...
NumPy's \li{np.meshgrid()} is the standard tool for creating a 2-dimensional domain in the Cartesian plane.
...
\begin{enumerate}
    ...
    \item Define a figure explicitly with \li{plt.fig()} and set its window boundaries.
