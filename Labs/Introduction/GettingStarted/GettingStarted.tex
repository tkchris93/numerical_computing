...
\begin{comment}
% TODO: for Python 3, insert a problem introducing the user to the print function more formally. This problem should be simple, but it should explore the keyword arguments 'sep' and 'end' included in the print function. (?)
\begin{problem}
...
\end{problem}
\end{comment} % Could wait until the file I/O lab to do this...
...
Common \li{list} methods (functions) include \li{append()}, \li{insert()}, \li{remove()}, and \li{pop()}.
...
\begin{enumerate}
\item \li{range(start, stop, step)}: Produces a list of integers, following slicing syntax. If only one argument is specified, it produces a list of integers from 0 to the argument, counting by one.
In Python 2.7, we use \li{xrange()} more often because it is faster\footnote{\li{xrange()} is a generator, whereas \li{range()} creates a \li{list}Generators will be briefly discussed in a future lab.} and does not explicitly create a list in memory.
\item \li{enumerate()}: Yields both a count and a value from the sequence. Typically used to get both the index of an item and the actual item simultaneously from a sequence.
...
# Get numbers from 2 to 20 (exclusive) by 3's:
>>> list(xrange(2, 20, 3))
[2, 5, 8, 11, 14, 17]
\end{lstlisting}
...
Translate the list of numbers into a list of strings, where each integer correspondings lower-case letter of the alphabet.
...
A \emph{list comprehension} uses \li{for} loop syntax between square brackets to create a \li{list}.
...
The following \li{for} loop and list comprehension produce the same list, but the list comprehension takes only about two-thirds the time to execute.
