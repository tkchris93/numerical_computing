...
% In the above example, we have a single function called \li{main()} that prints ``Hello, world!'' to the screen.
...
% However, it is good practice to write almost all code within functions, then call those functions under the \li{if __name__ == "__main__"} clause.

% Finally, each statement under the if __name__ == "__main__" clause is executed when the file is run. Any “orphan” statements that are not within a function definition, but not under the if __name__ == "__main__" clause, will also be executed when the file is run.
