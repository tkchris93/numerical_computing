...
% TODO: rename Introduction to Python (?)
\label{lab:Essential_Python}
\objective{Python is a powerful general-purpose programming language.
As an interpreted language it can be used interactively, allowing for very rapid development.
Python also has many powerful scientific computing tools, making it an ideal language for applied and computational mathematics.
Thus in today's technology-oriented economy, a thorough understanding of Python opens many doors to potential careers and makes many bleeding-edge technologies accessible.
\\ \indent
In this introductory lab we introduce Python syntax, data types, functions, and control flow tools.
These basics will be an essential part of every problem, algorithm, and program in this curriculum.
}
...
% \footnote{Do not be alarmed by the scientific programming community's apparent fixation with snakes. No snakes were harmed in the making of this software.}
...
Although later versions of Python are available, they do not yet have many of the libraries and features that we will require in this curriculum.
...
% TODO: shorten this section. Instead of talking about different options for IDEs, tell them to use a plain text editor in conjunction with a Unix shell.
Python files are saved with a \texttt{.py} extension.
For beginners, we strongly recommend using a simple text editor for programming in Python, though many free many free IDEs (Integrated Development Environments---programs that facilitate code development with some sophisticated tools) are also compatible with Python.
% Some of the more popular text editors are described in Appendix \ref{pythoninstall}.
...
Finally, actual executable Python code is written below the header.
In this file, we have a single function called \li{main()} that prints ``Hello, world!'' to the terminal.
Each statement under the \li{if __name__ == "__main__"} clause is executed when the file is run from the terminal.
Any statements that are not within a function definition, but not under the \li{if __name__ == "__main__"} clause, will also be executed when the file is run from the terminal.
It is good practice to write almost all code within functions, then call those functions under the \li{if __name__ == "__main__"} clause.
...
Below, the \li{\$} character indicates that the command is being run from the terminal. % in a Unix Shell.
...
\begin{info} % Difference between print and return
...
% TODO: Fix this problem! Specify input or say "Initialize the following list"
\begin{problem} % Problem: List operations.
...
% TODO: Replace with a Project Euler problem (largest palendropic number)
\begin{problem} % Problem: int_to_string().
