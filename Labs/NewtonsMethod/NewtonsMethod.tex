...
Use your function \li{Newtons_method()} to compute the zero of this function with a tolerance of $10^{-7}$.
\item Run \li{Newtons_method()} on $f(x)=x^9$ with $x_0=1$ and \li{tol}$=10^{-5}$. 
...
\item Run \li{plot_basins()} on the function $f(x) = x^3-1$ on the domain $\{a+bi \mid (a, b) \in [-1.5, 1.5] \times [-1.5, 1.5] \}$. 
