...
\end{lstlisting}
The fraction of points inside the circle is \li{numInCircle} divided by \li{numPoints}. By multiplying this fraction by the square's area, we can estimate the area of the circle. 
\begin{lstlisting}
...
\begin{problem}
\label{prob:sphere}
Write a function that estimates the volume of the unit sphere. Your function should have a keyword argument \li{numPoints} that defaults to $10^5$. Your function should draw \li{numPoints} points uniformly from $[-1,1]\times[-1,1]\times[-1,1]$ to make your estimate. The true volume is $\frac{4}{3}\pi \approx 4.189$.
\end{problem}
