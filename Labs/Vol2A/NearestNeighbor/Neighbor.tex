...
\objective{Nearest neighbor search is an optimization problem that arises in applications such as computer vision, pattern recognition, internet marketing, and data compression. In this lab we implement a k-d tree to solve the problem efficiently, then learn use SciPy's k-d tree from the \li{sklearn} module to implement a handwriting recognition algorithm.}
...
% Problem 3: KDTNode class
\begin{problem}
Copy or import the \li{BSTNode} class from the previous lab\footnote{If the file containing the \li{BSTNode} class is in a different directory than your solutions file for this lab, see \url{https://docs.python.org/2/tutorial/modules.html\#packages} for instructions on packages and imports.}.
...
\begin{comment} % Magic method was (outdated)
...
\Procedure{KDTSearch}{current, neighbor, distance}
\If {current is None (Base Case)}
    \State return neighbor, distance
\EndIf
	\If {distance between current and target $<$ distance (Update)}
		\State neighbor = current
		\State distance = distance between current and target
	\EndIf
    \If {target[current axis] $<$ current.value[current axis] (Recurse Left)}
    	\State neighbor, distance = KDTSearch(current.left, neighbor, distance)
    	\If {target[current axis] + distance $\geq$ current.value[current axis]}
    		\State neighbor, distance = KDTSearch(current.right, neighbor, distance)
    	\EndIf
    \Else { (Recurse Right)}
    	\State neighbor, distance = KDTSearch(current.right, neighbor, distance)
    	\If {target[current axis] - distance $\leq$ current.value[current axis]}
    		\State neighbor, distance = KDTSearch(current.left, neighbor, distance)
    	\EndIf
    \EndIf
\State return neighbor, distance
