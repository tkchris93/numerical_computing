...
% Finite Markov chains can be visualized as graphs where the nodes are the states and the connections are represented by the transition probabilities.
% The weather Markov chain can be visualized as follows.
% \begin{figure}[H]
% \includegraphics{width=\linewidth}[WEATHERCHAIN]
% \end{figure}
...
\For {each sentence in the training set}
    ...
    \State Add 1 to the entry of the transition matrix corresponding to the start state transitioning to the first word of the sentence.
   	\State For each consecutive pair $(i, j)$ of words in the list of words, add 1 to the entry of the transition matrix corresponding to moving from state $i$ to state $j$.
