...
The module \li{rsa_tools} contains some functions to fix this problem.
...
In fact, the function \li{string_size()} provided in the \li{rsa_tools} tells us the maximum number of characters we can encrypt with this choice of $n$.
...
Finally, write a \li{decrypt} method that decrypts a message with \li{_private_key}.
...
% Problem 2: Implement Fermat's test for primality.
\begin{problem}
...
The package is called \li{Crypto}, and can be downloaded \href{https://pypi.python.org/pypi/pycrypto}{here}.
...
% Problem 3: Implement a PyCrypto-powered RSA system.
\begin{problem}
Write a new RSA class called \li{PyCrypto} that uses methods from PyCrypto's \li{Crypto.RSA} package to accomplish the same objectives as the \li{myRSA} class.
...
The \li{decrypt} method decrypts a message using \li{_keypair}.
