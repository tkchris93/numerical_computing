...
The module \li{rsa_tools} contains some functions to fix this proble.
...
The \li{generate_keys} function accepts a pair of primes and sets \li{public_key} and \li{private_key} attributes.
The \li{encrypt} function accepts a string and returns the message (as a number) encrypted with the public key as the default.
If a different public key is provided then the message should be encrypted with the provided key.
The \li{decrypt} function decrypts a message using the private key.
The module \li{rsa_tools.py} provides methods for converting letters to numbers and vice-versa.
\label{prob:rsa1}
\end{problem}
...
The \li{share_this} variable may be distributed at a public key.
...
\begin{problem}
Modify the \li{RSA} class from the previous problems so that the user can choose whether or not to use \li{PyCrypto}.
To this end, add a key word argument to each method in the \li{RSA} called \li{pycrypto} that defaults to \li{False}.
If the user switches \li{pycrypto} to \li{True}, then use the \li{PyCrypto} methods to encrypt the message.
