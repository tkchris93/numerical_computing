...
\objective{Convey the importance of unit testing in code, especially in the corporate world. Unit testing in python is easy, valuable, and powerful. Help students understand how they can use unit testing to become more effective and productive programmers.}
...
which raises an AssertionError with error message \li{'message'} if \li{<truth statement>} is false, or else does nothing.
...
When run, \li{test_addition()} displays no output since all the tests are valid.
...
def func():
    ...
    return foo + bar + 42
\end{lstlisting}
and while debugging you reliaze that the value of \li{foo} was initialized wrong. Then after \li{foo} has been initialized you can enter the command
\begin{lstlisting}
(Pdb) foo = 5
\end{lstlisting}
